...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String token = fetchDeveloperToken();

        VoxeetSDK.initialize(
                token,
                (b, tokenCallback) -> {
                    tokenCallback.ok(token);
                }
        );
    ...
    private String fetchDeveloperToken() {
        if (Constants.API_TOKEN == "<REPLACE-WITH-YOUR-DEVELOPER-TOKEN>") {
            throw new AssertionError("You must add a valid developer token in Constants.java");
        }
